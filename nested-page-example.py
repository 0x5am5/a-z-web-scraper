...
for term in terms:
  print('Getting word: ' + term.text)

  # Get link to the term
  url = term.get('href')
  response = requests.get(url, timeout=5, headers=headers)
  nested_content = BeautifulSoup(response.content, "html.parser")

  # Use term from previous page or from the new page
  term = term.text
  # term = nested_content.find('h1', attrs={'class': 'entry-title'}).text

  # Get the definition
  definition = nested_content.find('div', attrs={'class': 'nested-content'}).text
